...
import BackToBlogBtn from "@/components/customs/BackToBlogBtn";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const imageUrl = post.coverImage ? `${siteUrl}${post.coverImage}` : `${siteUrl}/og-image-default.png`;
...
export default async function Page(props: { params: Promise<{ slug: string }> }): Promise<JSX.Element> {
  ...
  return (
    <>
      ...
      <main className="relative container flex flex-col mx-auto mt-8 mb-32 px-4">
        <BackToBlogBtn />
        <div className="relative flex flex-col xl:flex-row gap-12 justify-center items-start">
          <article className="w-full lg:max-w-2xl 2xl:max-w-5xl">
            {blogPost.coverImage && (
              <div className="relative w-full aspect-video mb-8 rounded-lg overflow-hidden">
                <Image src={blogPost.coverImage} alt={blogPost.title} fill className="object-fill" priority />
              </div>
            )}
